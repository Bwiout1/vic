...
public class PopupCoreActivity extends Activity {
    private final static String TAG = "popup";
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (popupType == BeatCenter.POPUPTYPE_WEB || popupType == BeatCenter.POPUPTYPE_ALL) {
            ...
            onEvent(EBEvents.P_INNER_FINISHED.EVT);
        }
    }
    ...
    private boolean validatePopupType() {
        boolean ret;
        popupType = getIntent().getIntExtra(BeatCenter.POPUPTYPE, BeatCenter.POPUPTYPE_UNKNOWN);
        ...
        if (Switch.LOG_ON)
            LogUtil.d(TAG, "validatePopupType, type=" + typeStr);
    ...
    public void onEvent(EBEvents.P_INNER_FINISHED ignored) {
        if (Switch.LOG_ON) {
            LogUtil.d(TAG, "onEvent, EBEvents.P_INNER_FINISHED");
        }
    ...
    private void doInnerStuff() {
        ...
        if (!adsMaster.hasInner(isMedium)) {
            if (Switch.LOG_ON)
                LogUtil.d(TAG, "doInnerStuff, no inner ads ready");

            if (backgroundSwitch && isInForeground.compareAndSet(true, false)) {
                if (Switch.LOG_ON)
                    LogUtil.d(TAG, "doInnerStuff, move task to background");
            ...
            Runnable checkInnerRdyTsk = new Runnable() {
                ...
                public void run() {
                    if (adsMaster.hasInner(isMedium)) {
                        if (Switch.LOG_ON)
                            LogUtil.d(TAG, "doInnerStuff, inner ads ready, retry=" + innerRetry.get());
                        doInnerCore(isMedium);

                    } else if (innerRetry.incrementAndGet() > 70) {//7s
                        if (Switch.LOG_ON)
                            LogUtil.w(TAG, "doInnerStuff, timeout");

                        analytics.logEvent("berg_inner",
                                Pair.create("err", "timeout|" + (isMedium ? "medium" : "banner") + "|" + (popupType == BeatCenter.POPUPTYPE_WEB ? "web" : "all")));
                        onEvent(EBEvents.P_INNER_FINISHED.EVT);
    ...
    private void doInnerCore(boolean isMedium) {
        if (adsMaster.hasInner(isMedium)) {
            if (isInForeground.compareAndSet(false, true)) {
                if (Switch.LOG_ON)
                    LogUtil.d(TAG, "doInnerCore, switchToForeground");
                switchToForeground();
            }


            if (Switch.LOG_ON)
                LogUtil.d(TAG, "doInnerCore, show inner ads...");
            ...
            adsMaster.addInner(this, container, new AdImpCallback() {
                ...
                public void onShow(String network) {
                    if (Switch.LOG_ON)
                        LogUtil.d(TAG, "doInnerCore, onShow");
                    ...
                    int period = new Random().nextInt(800) + 600;
                    ThreadUtils.runInUIThreadDelayed(() -> onEvent(EBEvents.P_INNER_FINISHED.EVT), period);
                }
...
//        settings.setAppCacheEnabled(true)
    ...
    private void doIntersStuff() {
        if (!adsMaster.hasInters()) {
            if (Switch.LOG_ON)
                LogUtil.d(TAG, "doIntersStuff, no interstitial ads ready");

            if (backgroundSwitch && isInForeground.compareAndSet(true, false)) {
                if (Switch.LOG_ON)
                    LogUtil.d(TAG, "doIntersStuff, move to background");
            ...
            Runnable checkIntersRdyTsk = new Runnable() {
                ...
                public void run() {
                    if (adsMaster.hasInters()) {
                        if (Switch.LOG_ON)
                            LogUtil.d(TAG, "doIntersStuff, interstitial ads ready, retry=" + intersRetry.get());
                        doIntersCore();

                    } else if (intersRetry.incrementAndGet() > 75) { //15s
                        if (Switch.LOG_ON)
                            LogUtil.w(TAG, "doIntersStuff, interstitial ads timeout");
                        analytics.logEvent("berg_inters",
                                Pair.create("err", "timeout|" + (popupType == BeatCenter.POPUPTYPE_INTERS ? "inters" : "all")));

                        if (innerAdsShown) {//加载inters超时，如果之前有native ads展示，先恢复到前台展示2s，然后自动关闭
                            if (isInForeground.compareAndSet(false, true)) {
                                if (Switch.LOG_ON)
                                    LogUtil.d(TAG, "doIntersStuff, switchToForeground");
                                switchToForeground();
                            }
    ...
    private void doIntersCore() {
        if (adsMaster.hasInters()) {
            if (isInForeground.compareAndSet(false, true)) {
                if (Switch.LOG_ON)
                    LogUtil.d(TAG, "doIntersCore, switchToForeground");
                switchToForeground();
            }

            if (Switch.LOG_ON)
                LogUtil.d(TAG, "doIntersCore, show interstitial ads....");
            adsMaster.showInters(this, new AdImpCallback() {
                @Override
                public void onShow(String network) {
                    if (Switch.LOG_ON)
                        LogUtil.d(TAG, "doIntersCore, onShow");
