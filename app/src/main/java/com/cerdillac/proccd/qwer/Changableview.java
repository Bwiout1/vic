...
public class Changableview extends androidx.appcompat.widget.AppCompatImageView {
    ...
    public Changableview(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public Changableview(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public void setShape_type(String type)
    {
        shape_type = type;
    }
    ...
    protected void onDraw(Canvas canvas){
        ...
        Bitmap shapeBitmap = drawShapedBitmap(cpBitmap, radius);
        canvas.drawBitmap(shapeBitmap, width / 2 - radius, height / 2 - radius,
                null);

    }
    //改变图形形状
    private  Bitmap drawShapedBitmap(Bitmap bmp,int radius){
        // TODO Auto-generated method stub
        ...
        if ("star".equals(shape_type)) {// 如果绘制的形状为五角星形
            ...
            path.close();
            canvas.drawPath(path, paint);
        } else if ("square".equals(shape_type)) {
            Path path = new Path();
