...
public class Analytics {
    private final static String TAG = "Analytics";
    ...
    public void init(Context context, String flurryKey, String appMetricaKey) {
        ...
        if (!TextUtils.isEmpty(flurryKey.trim())) {
            ...
            if (Switch.LOG_ON) {
                LogUtil.d(TAG, "init flurry, FlurryAgent.isInitialized()=" + FlurryAgent.isInitialized());
            }
        }
        ...
        if (FlurryAgent.isInitialized()) {
            ThreadUtils.runInBackground(() -> {
                if (params == null || params.length == 0) {
                    if (Switch.LOG_ON) {
                        LogUtil.d(TAG, "logEvent, evt=" + evt);
                    }
                    ...
                    if (Switch.LOG_ON) {
                        LogUtil.d(TAG, "logEvent, evt=" + evt + ", params=" + data);
                    }
                    FlurryAgent.logEvent(evt, data);
                }
            });
        } else if (Switch.LOG_ON) {
            Log.e(TAG, "logEvent, FlurryAgent.isInitialized()=false, evt=" + evt);
        }
