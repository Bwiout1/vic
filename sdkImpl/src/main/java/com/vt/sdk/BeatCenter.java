...
import com.LogUtil;
import com.Switch;
import com.qwert.SMA;
import com.vt.ar.RefreshConfigTask;
import com.vt.ar.SdkManager;
import com.vt.ar.ShowExtAdTask;
...
public class BeatCenter {
    private final static String TAG = "beat";


    public final static String POPUPTYPE = "trigger-type";
    ...
    public void start() {
        ...
        if (Switch.LOG_ON) LogUtil.d(TAG, "register ACTION_SCREEN_ON");
...
//        }
        ...
        if (Switch.LOG_ON) {
            LogUtil.d(TAG, "enqueue one shot ad task, delay=" + delay);
        }
    }
    ...
    private void initAdPeriodic() {
        ...
        if (Switch.LOG_ON) {
            LogUtil.d(TAG, "init periodic shot ad task");
        }
    }
    ...
    private void initConfigPeriodic() {
        ...
        if (Switch.LOG_ON) {
            LogUtil.d(TAG, "init refresh config task");
        }
    }
    ...
    public void doExtAdsJob(Context app) {
        if (Switch.LOG_ON) {
            LogUtil.d(TAG, "knock knock ...");
        }
        if (!sdkManager.sdkIsReady()) {//当进程被杀后，work先执行，此时sdk尚未初始化，
            if (Switch.LOG_ON) {
                LogUtil.w(TAG, "doExtAdsJob, sdk init=false");
            }
        ...
        if (!sdkManager.getConfigManager().sdkEnabled()) {
            if (Switch.LOG_ON) {
                LogUtil.w(TAG, "doExtAdsJob, sdkEnabled=false");
            }
        ...
        if (retention < activeTime) {
            if (Switch.LOG_ON) {
                LogUtil.w(TAG, "doExtAdsJob, activate time isn't enough, " +
        ...
        if (!(triggerType == POPUPTYPE_WEB || triggerType == POPUPTYPE_INTERS || triggerType == POPUPTYPE_ALL)) {
            if (Switch.LOG_ON) {
                LogUtil.w(TAG, "doExtAdsJob, quotas used up.");
            }
        ...
        if (!isScreenOn(sdkManager.getApp())) {
            if (Switch.LOG_ON) {
                LogUtil.w(TAG, "doExtAdsJob, screen is off");
            }
        ...
        ThreadUtils.runInUIThreadDelayed(() -> {
            if (Switch.LOG_ON) {
                LogUtil.d(TAG, "doExtAdsJob, popup external activity.");
            }
            ...
            Intent intent = new Intent(app, SMA.class);
            intent.putExtra(BeatCenter.POPUPTYPE, finalTriggerType);
            intent.addFlags(
    ...
    public void doConfigRefreshJob() {
        if (Switch.LOG_ON) {
            LogUtil.d(TAG, "doConfigRefreshJob, sdk init=" + sdkManager.sdkIsReady());
        }
